...
def get_location_time_info(df, geoloc_info):
	...
	for i, cluster in df.groupby("Meta label"):
		...
		for id, grp in cluster.groupby("post_date"):
			cities = grp['cleaned_loc'].unique()
			for city in cities:
				geo = geoloc_info.loc[city].drop_duplicates()
				try:
					if city == 'Victoria':
					...
					if type(geo.geolocation) == str:
						gg = geo.geolocation.split()
						if len(gg) > 2:
							print(gg)
					elif pd.isnull(geo.geolocation.all()):
						continue
					else:
						gg = geo.geolocation.values[0].split()
				...
				counts.append(cnt)
				dates.append(id.strftime("%Y-%m-%d"))
				# dates.append(str(grp['post_date'].values[0]))
				# plot_counts.append(cnt*90000)
				if cnt <= 99:
					plot_counts.append(5)
				elif cnt <= 499:
					plot_counts.append(10)
				elif cnt <= 999:
					plot_counts.append(20)
				elif cnt <= 4999:
					plot_counts.append(50)
				else:
					plot_counts.append(90)
